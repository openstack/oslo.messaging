...
class MetricsCollectorClient:
    ...
    def put_into_txqueue(self, metrics_name, action, **labels):
        ...
        m = message_type.Metric(
            "oslo_messaging", metrics_name, action, **labels
        )
        ...
        try:
            ...
            LOG.warning(
                "tx queues is already full(%d/%d). Fails to "
                "send the metrics(%s)",
                self.tx_queue.qsize(),
                self.tx_queue.maxsize,
                m,
            )
    ...
    def send_loop(self):
        ...
        while stoptime > time.time():
            ...
            try:
                ...
                LOG.error(
                    "Failed to send metrics: %s. Wait 1 second for next try.",
                    e,
                )
                time.sleep(1)
    ...
    def put_rpc_client_metrics_to_txqueue(
        self,
        metric_name,
        action,
        target,
    ...
    def rpc_client_invocation_start_total(
        self, target, method, call_type, timeout=None
    ):
    ...
    def rpc_client_invocation_end_total(
        self, target, method, call_type, timeout=None
    ):
        self.put_rpc_client_metrics_to_txqueue(
    ...
    def rpc_client_processing_seconds(
        self, target, method, call_type, duration, timeout=None
    ):
        self.put_rpc_client_metrics_to_txqueue(
    ...
    def rpc_client_exception_total(
        self, target, method, call_type, exception, timeout=None
    ):
        self.put_rpc_client_metrics_to_txqueue(
    ...
    def put_rpc_server_metrics_to_txqueue(
        self,
        metric_name,
        action,
        target,
    ...
    def rpc_server_invocation_start_total(
        self, target, endpoint, ns, ver, method
    ):
    ...
    def rpc_server_invocation_end_total(
        self, target, endpoint, ns, ver, method
    ):
        self.put_rpc_server_metrics_to_txqueue(
    ...
    def rpc_server_processing_seconds(
        self, target, endpoint, ns, ver, method, duration
    ):
        self.put_rpc_server_metrics_to_txqueue(
    ...
    def rpc_server_exception_total(
        self, target, endpoint, ns, ver, method, exception
    ):
        self.put_rpc_server_metrics_to_txqueue(
