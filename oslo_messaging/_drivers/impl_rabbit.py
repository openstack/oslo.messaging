...
rabbit_opts = [
    ...
    cfg.BoolOpt('ssl_enforce_hostname_verification',
                ...
                help='When true, verify the broker hostname against the '
                     ...
                     '(default on stable branches), ``ssl`` with '
                     '``ssl_ca_file`` still validates the certificate chain '
                     'but does not verify the broker hostname. ``ssl=true`` '
                     'without ``ssl_ca_file`` never enables hostname '
                     'verification.'),
