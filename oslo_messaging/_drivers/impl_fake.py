...
class FakeListener(base.PollStyleListener):
    ...
    def poll(self, timeout=None):
        ...
        while not self._stopped.is_set():
            for target in self._targets:
                exchange = self._exchange_manager.get_exchange(target.exchange)
                (ctxt, message, reply_q, requeue) = exchange.poll(
                    target, self._pool
                )
...
class FakeExchange:
    ...
    def deliver_message(
        self, topic, ctxt, message, server=None, fanout=False, reply_q=None
    ):
        with self._queues_lock:
            if fanout:
                queues = [
                    q for t, q in self._server_queues.items() if t[0] == topic
                ]
                ...
                self._get_topic_queue(topic)
                queues = [
                    q for t, q in self._topic_queues.items() if t[0] == topic
                ]
...
class FakeDriver(base.BaseDriver):
    ...
    def send(
        ...
        return self._send(
            target, ctxt, message, wait_for_reply, timeout, transport_options
        )
    ...
    def listen_for_notifications(
        ...
        targets = [
            oslo_messaging.Target(
                topic=f'{target.topic}.{priority}', exchange=target.exchange
            )
