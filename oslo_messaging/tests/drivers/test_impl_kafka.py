...
class TestKafkaTransportURL(test_utils.BaseTestCase):
    scenarios = [
        (
            'port',
            dict(
                url='kafka://localhost:1234',
                expected=dict(
                    hostaddrs=['localhost:1234'],
                    username=None,
                    password=None,
                    vhost=None,
                ),
        ...
        (
            'vhost',
            dict(
                url='kafka://localhost:1234/my_host',
                expected=dict(
                    hostaddrs=['localhost:1234'],
                    username=None,
                    password=None,
        ...
        (
            'two',
            dict(
                url='kafka://localhost:1234,localhost2:1234',
                expected=dict(
                    hostaddrs=['localhost:1234', 'localhost2:1234'],
                    username=None,
                    password=None,
                    vhost=None,
                ),
        ...
        (
            'user',
            dict(
                url='kafka://stack:stacksecret@localhost:9092/my_host',
                expected=dict(
                    hostaddrs=['localhost:9092'],
                    username='stack',
        ...
        (
            'user2',
            dict(
                url='kafka://stack:stacksecret@localhost:9092,'
                'stack2:stacksecret2@localhost:1234/my_host',
                expected=dict(
                    hostaddrs=['localhost:9092', 'localhost:1234'],
                    username='stack',
                    password='stacksecret',
                    vhost='my_host',
                ),
        ...
        (
            'ipv4',
            dict(
                url='kafka://127.0.0.1:1234',
                expected=dict(
                    hostaddrs=['127.0.0.1:1234'],
                    username=None,
                    password=None,
                    vhost=None,
                ),
...
class TestKafkaDriver(test_utils.BaseTestCase):
    ...
    def test_send(self):
        ...
        self.assertRaises(
            NotImplementedError, self.driver.send, target, {}, {}
        )
    ...
    def test_send_notification(self):
        ...
        with mock.patch("confluent_kafka.Producer") as producer:
            self.driver.send_notification(
                target, {}, {"payload": ["test_1"]}, None, retry=3
            )
    ...
    def test_listen(self):
        target = oslo_messaging.Target(topic="topic_test")
        self.assertRaises(
            NotImplementedError, self.driver.listen, target, None, None
        )
