...
class TestHeartbeat(test_utils.BaseTestCase):
    ...
    def _do_test_heartbeat_sent(
        self,
        fake_ensure_connection,
        fake_heartbeat_support,
        fake_heartbeat,
        fake_logger,
        ...
        if not heartbeat_side_effect:
            ...
            self.assertIn(
                mock.call(info, mock.ANY), fake_logger.info.mock_calls
            )
...
class TestRabbitQos(test_utils.BaseTestCase):
    def connection_with(self, prefetch, purpose):
        self.config(
            rabbit_qos_prefetch_count=prefetch, group="oslo_messaging_rabbit"
        )
        transport = oslo_messaging.get_transport(
            self.conf, 'kombu+memory:////'
        )
...
class TestRabbitDriverLoad(test_utils.BaseTestCase):
    ...
    def test_driver_load(self, fake_ensure, fake_reset):
        self.config(
            heartbeat_timeout_threshold=60, group='oslo_messaging_rabbit'
        )
...
class TestRabbitDriverSSLHostname(test_utils.BaseTestCase):
    ...
    def test_multi_host_host(self, connection_klass, fake_ensure):
        self.config(
            ssl=True, ssl_ca_file='foobar', group='oslo_messaging_rabbit'
        )
        transport = oslo_messaging.get_transport(
    ...
    @mock.patch('kombu.connection.Connection')
    def test_ssl_without_ca_does_not_check_hostname(
...
class TestRabbitPublisher(test_utils.BaseTestCase):
    ...
    def test_send_with_timeout(self, fake_publish):
        ...
        with transport._driver._get_connection(
            ...
            conn = pool_conn.connection
            conn._publish(
                exchange_mock, 'msg', routing_key='routing_key', timeout=1
            )
    ...
    def test_declared_queue_publisher(self):
        ...
        e_passive = kombu.entity.Exchange(
            name='foobar', type='topic', passive=True
        )

        e_active = kombu.entity.Exchange(
            name='foobar', type='topic', passive=False
        )
        ...
        with transport._driver._get_connection(
            ...
            with mock.patch('kombu.transport.virtual.Channel.close'):
                ...
                self.assertRaises(
                    oslo_messaging.MessageDeliveryFailure, try_send, e_passive
                )
                # Create it
                try_send(e_active)
            ...
            with mock.patch(
                'kombu.messaging.Producer.publish', side_effect=exc
            ):
                with mock.patch('kombu.transport.virtual.Channel.close'):
    ...
    def test_send_exception_remap(self):
        ...
        with transport._driver._get_connection(
            ...
            conn = pool_conn.connection
            with mock.patch(
                'kombu.messaging.Producer.publish', side_effect=bad_exc
            ):
                self.assertRaises(
...
class TestRabbitConsume(test_utils.BaseTestCase):
    def test_consume_timeout(self):
        ...
        with transport._driver._get_connection(
            ...
            with mock.patch(
                'kombu.connection.Connection.drain_events', side_effect=IOError
            ):
                self.assertRaises(
                    driver_common.Timeout, conn.consume, timeout=3
                )
    ...
    def test_consume_from_missing_queue(self):
        ...
        with transport._driver._get_connection(
            ...
            with (
                mock.patch('kombu.Queue.consume') as consume,
                mock.patch('kombu.Queue.declare') as declare,
            ):
    ...
    def test_connection_ack_have_disconnected_kombu_connection(self):
        ...
        with transport._driver._get_connection(
            ...
            with mock.patch(
                'kombu.connection.Connection.connected',
                new_callable=mock.PropertyMock,
                return_value=False,
            ):
                self.assertRaises(
                    driver_common.Timeout,
                    conn.connection.consume,
                    timeout=0.01,
                )
    ...
    def test_heartbeat_check_only_on_timeout(self):
        ...
        with transport._driver._get_connection(
            ...
            with mock.patch.object(
                conn.connection, '_heartbeat_check'
            ) as mock_hb:
                conn.connection.connection.drain_events = mock.Mock(
                    return_value=None
                )
...
class TestRabbitTransportURL(test_utils.BaseTestCase):
    scenarios = [
        (
            'none',
            dict(url=None, expected=["amqp://guest:guest@localhost:5672/"]),
        ),
        ('memory', dict(url='kombu+memory:////', expected=["memory:///"])),
        (
            'empty',
            dict(
                url='rabbit:///',
        ...
        (
            'no_port',
            dict(
                url='rabbit://user:password@host/virtual_host',
        ...
        (
            'full_url',
            dict(
                url='rabbit://user:password@host:10/virtual_host',
        ...
        (
            'full_two_url',
            dict(
                url='rabbit://user:password@host:10,'
                'user2:password2@host2:12/virtual_host',
                expected=[
                    "amqp://user:password@host:10/virtual_host",
        ...
        (
            'rabbit_ipv6',
            dict(
                url='rabbit://u:p@[fd00:beef:dead:55::133]:10/vhost',
        ...
        (
            'rabbit_ipv4',
            dict(
                url='rabbit://user:password@10.20.30.40:10/vhost',
        ...
        (
            ...
            dict(
                url='rabbit://user:password@10.20.30.40:10',
    ...
    def setUp(self):
        ...
        self.config(
            heartbeat_timeout_threshold=0, group='oslo_messaging_rabbit'
        )
...
class TestSendReceive(test_utils.BaseTestCase):
    ...
    _timeout = [
        ...
        (
            'timeout',
            dict(
                timeout=0.01,  # FIXME(markmc): timeout=0 is broken?
                call_monitor_timeout=None,
            ),
        ),
        (
            'call_monitor_timeout',
            dict(timeout=0.01, call_monitor_timeout=0.02),
        ),
    ]
    ...
    def test_send_receive(self):
        self.config(
            kombu_missing_consumer_retry_timeout=0.5,
            group="oslo_messaging_rabbit",
        )
        self.config(
            heartbeat_timeout_threshold=0, group="oslo_messaging_rabbit"
        ...
        while len(senders) < self.n_senders:
            senders.append(
                threading.Thread(
                    target=send_and_wait_for_reply, args=(len(senders),)
                )
            )
    ...
    def test_send_receive(self):
        ...
        def reply_waiter(self, msg_id, timeout, call_monitor_timeout, reply_q):
            if wait_conditions:
                ...
                with cond:
                    cond.wait()
            return orig_reply_waiter(
                self, msg_id, timeout, call_monitor_timeout, reply_q
            )
        ...
        while len(senders) < 2:
            t = threading.Thread(
                target=send_and_wait_for_reply, args=(len(senders), True)
            )
            t.daemon = True
            senders.append(t)
        ...
        t = threading.Thread(
            target=send_and_wait_for_reply, args=(len(senders), False)
        )
        t.daemon = True
        senders.append(t)
...
def _declare_queue(target):
    ...
    if target.fanout:
        exchange = kombu.entity.Exchange(
            name=target.topic + '_fanout',
            type='fanout',
            durable=False,
            auto_delete=True,
        )
        queue = kombu.entity.Queue(
            name=target.topic + '_fanout_12345',
            channel=channel,
            exchange=exchange,
        ...
        queue = kombu.entity.Queue(
            name=topic, channel=channel, exchange=exchange, routing_key=topic
        )
    else:
        exchange = kombu.entity.Exchange(
            name='openstack', type='topic', durable=False, auto_delete=False
...
class TestRequestWireFormat(test_utils.BaseTestCase):
    _target = [
        ('topic_target', dict(topic='testtopic', server=None, fanout=False)),
        (
            'server_target',
            dict(topic='testtopic', server='testserver', fanout=False),
        ),
        ('fanout_target', dict(topic='testtopic', server=None, fanout=True)),
    ]

    _msg = [
        ('empty_msg', dict(msg={}, expected={})),
        ('primitive_msg', dict(msg={'foo': 'bar'}, expected={'foo': 'bar'})),
        (
            'complex_msg',
            dict(
                msg={'a': {'b': datetime.datetime(1920, 2, 3, 4, 5, 6, 7)}},
    ...
    _context = [
        ...
        (
            'user_project_ctxt',
            dict(
                ctxt={'user': 'mark', 'project': 'snarkybunch'},
                expected_ctxt={
                    '_context_user': 'mark',
    ...
    def generate_scenarios(cls):
        cls.scenarios = testscenarios.multiply_scenarios(
            cls._msg, cls._context, cls._target, cls._compression
        )
    ...
    def test_request_wire_format(self):
        ...
        target = oslo_messaging.Target(
            topic=self.topic, server=self.server, fanout=self.fanout
        )
...
def _create_producer(target):
    ...
    if target.fanout:
        ...
        producer = kombu.messaging.Producer(
            exchange=exchange, channel=channel, routing_key=target.topic
        )
    elif target.server:
        exchange = kombu.entity.Exchange(
            name='openstack', type='topic', durable=False, auto_delete=False
        ...
        topic = f'{target.topic}.{target.server}'
        producer = kombu.messaging.Producer(
            exchange=exchange, channel=channel, routing_key=topic
        )
    else:
        exchange = kombu.entity.Exchange(
            name='openstack', type='topic', durable=False, auto_delete=False
...
class TestReplyWireFormat(test_utils.BaseTestCase):
    ...
    _msg = [
        ...
        (
            'complex_msg',
            dict(
                msg={'a': {'b': '1920-02-03T04:05:06.000007'}},
                expected={'a': {'b': '1920-02-03T04:05:06.000007'}},
            ),
        ),
    ]
...
class RpcKombuHATestCase(test_utils.BaseTestCase):
    def setUp(self):
        ...
        self.connection = rabbit_driver.Connection(
            self.conf, url, driver_common.PURPOSE_SEND
        )
        self.useFixture(
...
class TestMsgIdCache(test_utils.BaseTestCase):
    ...
    def test_reply_wire_format(self, reject_mock):
        ...
        target = oslo_messaging.Target(
            topic='testtopic', server=None, fanout=False
        )
