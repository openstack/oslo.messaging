...
class TestParseURL(test_utils.BaseTestCase):
    scenarios = [
        ('transport', dict(url='foo:', expect=dict(transport='foo'))),
        (
            'virtual_host_slash',
            dict(
                url='foo:////', expect=dict(transport='foo', virtual_host='/')
            ),
        ),
        (
            'virtual_host',
        ...
        (
            'password',
            dict(
                url='foo://u:p@host:1234/bar',
                expect=dict(
                    transport='foo',
                    virtual_host='bar',
                    hosts=[
                        dict(
                            host='host', port=1234, username='u', password='p'
                        ),
                    ],
                ),
            ),
        ...
        (
            'multi_host',
            dict(
                url='foo://u:p@host1:1234,host2:4321/bar',
                expect=dict(
                    transport='foo',
                    virtual_host='bar',
                    hosts=[
                        dict(
                            host='host1', port=1234, username='u', password='p'
                        ),
        ...
        (
            'multi_creds',
            dict(
                url='foo://u1:p1@host1:1234,u2:p2@host2:4321/bar',
                expect=dict(
                    transport='foo',
                    virtual_host='bar',
                    hosts=[
        ...
        (
            'multi_creds_ipv6',
            dict(
                url='foo://u1:p1@[ffff::1]:1234,u2:p2@[ffff::2]:4321/bar',
                expect=dict(
                    transport='foo',
                    virtual_host='bar',
                    hosts=[
        ...
        (
            'quoting',
            dict(
                url='foo://u%24:p%26@host:1234/%24',
                expect=dict(
                    transport='foo',
                    virtual_host='$',
                    hosts=[
    ...
    def test_parse_url(self):
        ...
        expected = oslo_messaging.TransportURL(
            self.conf,
...
class TestFormatURL(test_utils.BaseTestCase):
    scenarios = [
        ...
        (
            ...
            dict(
                ...
                hosts=[
                    dict(
                        hostname='h1', port=1000, username='b1', password='s1'
                    ),
                    dict(
                        hostname='h2', port=2000, username='b2', password='s2'
                    ),
                ],
        ...
        (
            ...
            dict(
                ...
                hosts=[
                    dict(
                        hostname='host', port=10, username='b$', password='s&'
                    ),
                ],
    ...
    def test_parse_url(self):
        ...
        url = oslo_messaging.TransportURL(
            self.conf, self.transport, self.virtual_host, hosts
        )
