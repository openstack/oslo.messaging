...
class TargetCallableTestCase(test_utils.BaseTestCase):
    scenarios = [
        ...
        (
            'exchange_attr',
            dict(
                attrs=dict(exchange='testexchange'),
                kwargs=dict(),
        ...
        (
            'namespace_attr',
            dict(
                attrs=dict(namespace='testnamespace'),
                kwargs=dict(),
        ...
        (
            'fanout_attr',
            dict(
                attrs=dict(fanout=True), kwargs=dict(), vals=dict(fanout=True)
            ),
        ),
        (
            'fanout_arg',
            dict(
                attrs=dict(), kwargs=dict(fanout=True), vals=dict(fanout=True)
            ),
        ),
    ]
...
class TargetReprTestCase(test_utils.BaseTestCase):
    scenarios = [
        ...
        (
            'topic',
            dict(kwargs=dict(topic='testtopic'), repr='topic=testtopic'),
        ),
        (
            'namespace',
            dict(
        ...
        ('version', dict(kwargs=dict(version='3.4'), repr='version=3.4')),
        (
            'server',
            dict(kwargs=dict(server='testserver'), repr='server=testserver'),
        ),
        ('fanout', dict(kwargs=dict(fanout=True), repr='fanout=True')),
        (
            'exchange_and_fanout',
            dict(
                kwargs=dict(exchange='testexchange', fanout=True),
                repr='exchange=testexchange, fanout=True',
            ),
        ),
    ]
