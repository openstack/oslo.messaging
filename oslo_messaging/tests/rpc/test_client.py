...
class TestCastCall(test_utils.BaseTestCase):
    ...
    def test_cast_call(self):
        ...
        transport._send.assert_called_once_with(
            oslo_messaging.Target(), self.ctxt, msg, **kwargs
        )
...
class TestCastToTarget(test_utils.BaseTestCase):
    _base = [
        ...
        (
            'ctor_exchange',
            dict(
                ctor=dict(exchange='testexchange'),
                prepare={},
        ...
        (
            'both_exchange',
            dict(
                ctor=dict(exchange='ctorexchange'),
                prepare=dict(exchange='testexchange'),
                expect=dict(exchange='testexchange'),
            ),
        ),
        ...
        (
            'ctor_namespace',
            dict(
                ctor=dict(namespace='testnamespace'),
                prepare={},
        ...
        (
            'both_namespace',
            dict(
                ctor=dict(namespace='ctornamespace'),
                prepare=dict(namespace='testnamespace'),
                expect=dict(namespace='testnamespace'),
            ),
        ),
        ...
        (
            ...
            dict(
                ctor=dict(version='1.1'), prepare=dict(version=None), expect={}
            ),
        ),
        (
            'both_version',
            dict(
                ctor=dict(version='ctorversion'),
                prepare=dict(version='1.1'),
                expect=dict(version='1.1'),
            ),
        ),
        ...
        (
            'ctor_fanout',
            dict(ctor=dict(fanout=True), prepare={}, expect=dict(fanout=True)),
        ),
        (
            'prepare_fanout',
            dict(ctor={}, prepare=dict(fanout=True), expect=dict(fanout=True)),
        ),
        (
            'prepare_fanout_none',
            dict(ctor=dict(fanout=True), prepare=dict(fanout=None), expect={}),
        ),
        (
            'both_fanout',
            dict(
                ctor=dict(fanout=True),
...
class TestCallTimeout(test_utils.BaseTestCase):
    scenarios = [
        (
            'all_none',
            dict(
                confval=None, ctor=None, prepare=_notset, expect=None, cm=None
            ),
        ),
        (
            'confval',
        ...
        (
            'ctor',
            dict(
                confval=None, ctor=21.1, prepare=_notset, expect=21.1, cm=None
            ),
        ),
        (
            'ctor_zero',
    ...
    def test_call_timeout(self):
        ...
        transport._send.assert_called_once_with(
            oslo_messaging.Target(), {}, msg, **kwargs
        )
...
class TestCallRetry(test_utils.BaseTestCase):
    ...
    def test_call_retry(self):
        transport = oslo_messaging.get_rpc_transport(self.conf, url='fake:')
        client = oslo_messaging.get_rpc_client(
            transport, oslo_messaging.Target(), retry=self.ctor
        )
...
class TestVersionCap(test_utils.BaseTestCase):
    ...
    def generate_scenarios(cls):
        cls.scenarios = testscenarios.multiply_scenarios(
            cls._call_vs_cast, cls._cap_scenarios
        )
    ...
    def test_version_cap(self):
        ...
        client = oslo_messaging.get_rpc_client(
            transport, target, version_cap=self.cap
        )
...
class TestCanSendVersion(test_utils.BaseTestCase):
    ...
    def test_invalid_version_type(self):
        ...
        client = oslo_messaging.get_rpc_client(transport, target)
        self.assertRaises(
            exceptions.MessagingException, client.prepare, version='5'
        )
        self.assertRaises(
            exceptions.MessagingException, client.prepare, version='5.a'
        )
        self.assertRaises(
            exceptions.MessagingException, client.prepare, version='5.5.a'
        )
...
class TestClientMetrics(test_utils.BaseTestCase):
    @mock.patch.object(
        metrics_client.MetricsCollectorClient, "put_into_txqueue"
    )
    def test_metrics(self, mock_put):
        ...
        self.client = oslo_messaging.RPCClient(
            transport, target, transport_options=transport_options
        )
