...
class PublishErrorsHandlerTestCase(test_utils.BaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.publisherrorshandler = log_handler.PublishErrorsHandler(
            logging.ERROR
        )
    ...
    def test_emit_notification(self, mock_notify):
        ...
        self.publisherrorshandler.emit(logrecord)
        self.assertEqual(
            'error.publisher', self.publisherrorshandler._notifier.publisher_id
        )
        mock_notify.assert_called_with(
            {}, 'error_notification', {'error': 'Message'}, 'ERROR'
        )
